...
import { getSmarkEvents } from "./repository";

export const processEvents = async (bookmarks: BookmarkListWithChildren[]) => {
    const pendingEvents = await getSmarkEvents();
    for (let event of pendingEvents) {
        switch (event.type) {
                ...
                bookmarks.forEach((list) => {
                    if (list.title.trim() === data.listTitle.trim()) {
                        list.children.forEach((bookmark) => {
                            if (
                                bookmark.title.trim() === data.oldTitle.trim()
                            ) {
            ...
            case "create_list": {
                const data = event.data;
                if (
                    bookmarks.find((l) => l.title.trim() === data.title.trim())
                ) {
                    break;
                }
                bookmarks.push({
